...
!-------------------------------------------------------------
  ...
  write(*,*) ' my node =', node,' Path=',Path                                              
  If(node == 1)Then
    write(*,'(a,$)') ' Enter realization number for this run = '
    Read(*,*) Nrealization
    write(*,*) ' my starting realization =',Nrealization,node
  End If    
  CALL MPI_BCAST(Nrealization,  1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
  Nrealization = Nrealization + node
  write(*,*) ' my realization after shift =',Nrealization,node
  
      write(Path,'(a,i4.4,a)')Trim(Path),Nrealization,'/'
      write(*,*) ' New path= ',Path
  write(Line,*)' mkdir ',TRIM(Path)
  write(*,*) ' system comand= ',Trim(Line)
...
include 'mpif.h'     
     logical :: exst
      ...
      if(.not.exst)Then
         ...
         Stop ' File PkTable with the power spectrum not found'
      EndIf
...
!        field -- see PMP2init.f90's ParseLine for the full rationale.
...
!                                     Open control file               
      OPEN(9, FILE = Path//'PMcrd.DAT', form = 'unformatted') 
                            ! this clears old header file               
      Write(9) 
      CLOSE (9) 
      OPEN(9, FILE = Path//'PMcrd.DAT', FORM = 'UNFORMATTED', STATUS =       &
         'UNKNOWN')                                                        
      OPEN(16, FILE = Path//'Results.log') 
      OPEN(25, file = Path//'pt.dat', form = 'unformatted') 
...
!                                       write header and control data   
   ...
   character(len=*) :: Path 
      ...
      Do i=1,Nfiles                       !-- open files
         ...
         EndIf
         Open(30+ifile,file=Path//TRIM(Fname),ACCESS='DIRECT', &
...
PROGRAM  PMstartMp
  ...
  node = irank +1
  write(Path,'(a)')'Sim'
      write (*,'(a,2i4,a)')  '  MPI:  size,node=', mpisize,node,TRIM(Path) 
   CALL Timing(0,-1)
   CALL Timing(1,-1)
   ...
   write(*,*) ' my realization after shift =',Nrealization,node
     ...
     If(iWrite==1)Then      
        CALL FilesWrite(Trim(Path))           ! write header and control data
