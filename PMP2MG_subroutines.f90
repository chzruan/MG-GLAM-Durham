...
!     use AMLUtils
      integer n, ind, nw, k
      real*8 :: x, y(n), xend, tol, c(*), w(nw,9), temp
      real*8 :: EV !It isn't, but as long as it maintains it as a pointer we are OK
...
!  .........abort if n.gt.nw or tol.le.0
            if (n.gt.nw .or. tol.le.0.0d0) go to 500
...
!              any, else default
               c(14) = c(4)
               if (c(4) .eq. 0.0d0) c(14) = c(16)*tol**(1.0d0/6.0d0)
               go to 185
...
!
  ...
  200       continue
            call fcn(EV,n, x + c(18)/6.0d0, w(1,9), w(1,2))
!
            do 205 k = 1, n
               w(k,9) = y(k) + temp*(   w(k,1)*74569017600.0d0 &
                                      + w(k,2)*298276070400.0d0  )
  205       continue
            call fcn(EV,n, x + c(18)*(4.0d0/15.0d0), w(1,9), w(1,3))
!
            do 210 k = 1, n
  ...
  210       continue
            call fcn(EV,n, x + c(18)*(2.0d0/3.0d0), w(1,9), w(1,4))
!
            do 215 k = 1, n
  ...
  215       continue
            call fcn(EV,n, x + c(18)*(5.0d0/6.0d0), w(1,9), w(1,5))
!
            do 220 k = 1, n
...
!
  ...
  225       continue
            call fcn(EV,n, x + c(18)/15.0d0, w(1,9), w(1,7))
!
            do 230 k = 1, n
