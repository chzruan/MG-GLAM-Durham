...
Program Read
    ...
    Integer*4     :: SnapNum, ii, Nargs, iarg, Npos
    Character*200 :: SnapNumArg, ArgBuf, PosArg(3)
    Character*8   :: LengthUnit

    !--- Positional args and the optional --units= flag may be interleaved, so
    !    collect them in one pass rather than indexing on Command_Argument_Count.
    LengthUnit = 'Mpc/h'    ! (MG-)GLAM's own unit; see WriteGadgetFormat.f90
    Npos  = 0
    Nargs = Command_Argument_Count()

    DO iarg = 1, Nargs
        CALL Get_Command_Argument(iarg, ArgBuf)
        IF(ArgBuf(1:8) == '--units=') THEN
            SELECT CASE (TRIM(ArgBuf(9:)))
            CASE ('Mpc/h','mpc/h','MPC/H','Mpc','mpc','MPC')
                LengthUnit = 'Mpc/h'
            CASE ('kpc/h','kPc/h','KPC/H','kpc','Kpc','KPC')
                LengthUnit = 'kpc/h'
            CASE DEFAULT
                WRITE(*,*) ' Unrecognised --units value: ', TRIM(ArgBuf(9:))
                Error Stop 'Use --units=Mpc/h (default) or --units=kpc/h'
            END SELECT
        ELSE IF(ArgBuf(1:2) == '--') THEN
            WRITE(*,*) ' Unrecognised option: ', TRIM(ArgBuf)
            Error Stop 'Unrecognised command-line option'
        ELSE
            Npos = Npos + 1
            IF(Npos > 3) Error Stop 'Too many positional command-line arguments'
            PosArg(Npos) = ArgBuf
        ENDIF
    END DO

    IF(Npos < 2) THEN
        WRITE(*,*) 'Usage: glam2gadget.exe <GLAMFolderPath/> <SnapNum> [OutputGadgetFilePath] [--units=Mpc/h|kpc/h]'
        ...
        WRITE(*,*) '   --units        : length unit of the positions and box size WRITTEN'
        WRITE(*,*) '                    to the Gadget file. Default Mpc/h, which is what'
        WRITE(*,*) '                    (MG-)GLAM uses throughout; kpc/h is the stock'
        WRITE(*,*) '                    Gadget convention. A reader that assumes the other'
        WRITE(*,*) '                    one is off by exactly 1000 and will not complain.'
        Error Stop 'Wrong number of command-line arguments'
    ENDIF

    GLAMFolderPath = PosArg(1)
    SnapNumArg     = PosArg(2)
...
!---------------------------------------------------
    ...
    write(*,*) 'x,y,z min=',minval(pos), ' ', TRIM(LengthUnit)
    write(*,*) 'x,y,z max=',maxval(pos), ' ', TRIM(LengthUnit)
