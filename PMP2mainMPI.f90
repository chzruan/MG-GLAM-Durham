!-------------------------------------------------
!            Parallel (OMP + MPI) PM code
!	     2016, 1997  Anatoly Klypin (aklypin@nmsu.edu)
...
Program PMP
  ...
  write(Path,'(a)')'Sim'
  Path =TRIM(Path)
  if(node==1)write (*,'(a,i3,2a)')  '  MPI:  size=', mpisize,' Path =',Trim(Path) 

  Ncheckpoint  = 500   !  save for re-starting every  Ncheckpoint step

  Call Initialize(Path)     
  write (17,'(a,i3,a,i3)') ' node= ',node,' iBDM= ',iBDM 
      DO  i=1,Nsteps         !  Main loop
         Call TimingMain(0,-1)
         CALL DENSIT            ! Define density
         CALL POTENTfft5            ! Define potential
         FactV  = 1.                ! normal step
         If(ASTEP<StepFactor/1.25*AEXPN.and.AEXPN<0.30)Then
...
!--------------------------------------------
  ...
  character(len=*) :: Path 
   ...
   If(node==1)Then
      ...
      WRITE (*,'(A,$)') ' Enter name XXXX of first data directory SimXXXX => '
      READ (*,*) Nsub	 ! 
   ...
   End If
      ...
      CALL MPI_BCAST(NbiasPars,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(Nsteps,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)

      CALL MPI_BCAST(ASTEP,    1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(AEXPN0,    1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(ASTEP0,   1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(AMPLT,    1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(sigma8,   1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      ...
      CALL MPI_BCAST(zfinal,   1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(DensThr,  1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(sigV,     1,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(zout,     Nout,MPI_REAL,0,MPI_COMM_WORLD,ierr)      
      CALL MPI_BCAST(BiasPars, Nbiaspars,MPI_REAL,0,MPI_COMM_WORLD,ierr)     
      CALL MPI_BCAST(Nsub,      1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iSave,     1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iPower,    1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iPowerRSD, 1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iDensityDistr, 1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iBias,     1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iWriteMock,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(iBDM,      1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)

      Nsub = Nsub + node
      Nrealization = Nsub
      write(Path,'(a,i4.4,a)')Trim(Path),Nsub,'/'
            write(*,*) ' New path= ',Path
      OPEN(17,FILE=TRIM(Path)//'Run.log',STATUS='UNKNOWN',position='append')

      CALL ReadDataPM(-1,Path)
      write(Line,'(3a)')"rm -f "//TRIM(Path)//"PMcr*DAT"
      write(*,*) TRIM(Line)
      Call System(TRIM(Line))      !--- remove files after reading them
      If(iBDM==1)Then

        ! write(Line,'(3a)')"cp -f BDM.config "//TRIM(Path)//"."
        ! write(*,'(a)') TRIM(Line)         
      ...
      write(17,'(a,T20,a,T30,i12,T45,a,i4)')' Start running: ', &
                      'Nparticles:',Nparticles,' Ngrid= ',Ngrid
      write(17,'(T20,a,T30,i12,T45,a,es12.4)')  'Step:',ISTEP,' da=',ASTEP
      write(17,'(T20,a,T30,es12.4)')  'StepFactor :',StepFactor
      write(17,'(T20,a,T30,i3)')  'BDM        :',iBDM
      ...
      do i=1,Ntotal
         if(Nlist(i)==1)&
         write(17,'(i5,2es13.4,i3)') i,Alist(i),1./Alist(i)-1.,Nlist(i)
      endDo
